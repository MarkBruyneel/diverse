...
#
# CoinGecko’s Public API has a rate limit of 5 to 15 calls per minute,
...
# Set list of valid currencies covered by CoinGecko at 2025-10-03.
# Normally I would use a request for this but this is only available for paid users.
curr_list = ['aed', 'ars', 'aud', 'bch', 'bdt', 'bhd', 'bits', 'bmd', 'bnb', 'brl', 'btc', 'cad', 'chf', 'clp', 'cny', 'czk', 'dkk', 'dot', 'eos', 'eth', 'eur', 'gbp', 'gel', 'hkd', 'huf', 'idr', 'ils', 'inr', 'jpy', 'krw', 'kwd', 'link', 'lkr', 'ltc', 'nmk', 'mxn', 'myr', 'ngn', 'nok', 'nzd', 'php', 'pkr', 'pln', 'rub', 'sar', 'sats', 'sek', 'sgd', 'sol', 'thb', 'try', 'twd', 'uah', 'usd', 'vef', 'vnd', 'xag', 'xau', 'xdr', 'xlm', 'xrp', 'yfi', 'zar']
...
def main():
    ...
    # Check input for a valid currency currently covered by CoinGecko
    curr_choice = ''
    while curr_choice not in curr_list:
        print('Currencies covered by CoinGecko:\n', curr_list)
        curr_choice = (input('\nWhich currency do you want data for?')).lower()
        if curr_choice in curr_list:
            continue
        else:
    ...
    try:
        while datenr < start:
            ...
            try:
                ...
                market = data['market_data']['market_cap'][curr_choice]
            except KeyError:
                market = "None"
            try:
                # Test if the volume field is missing
                volume = data['market_data']['total_volume'][curr_choice]
            except KeyError:
                volume = "None"
        ...
        temp_table[f'{curr_choice}_Price'] = price_list
